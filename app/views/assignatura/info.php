...
<div data-url-refresh="<?php echo URL::route('assignatura.noves', array('idAssignatura'=>$assignatura->getID()))?>"></div>
	<div class="capcalera">	  
		<h2 style="float: left;"><?php echo $assignatura->getNom() ?></h2>
		...
		<?php
			...
			if($moderador->getEmail() == Auth::user()->email_estudiant){
				$slug = $assignatura->getSlug();
				?>
				
				<?php
				/*Estil depenent de la longitud del grup*/
					if (strlen($assignatura->getNom()) > 25){
				...
				<div class="moderar modH1" style="<?php echo $estil ?>">
					<a href="<?php echo URL::route('assignatura.editar', array('slug' => $slug))?>">
						<img src="/laravel/public/img/edit3.png" 
						alt="Moderar" 
						data-toggle="tooltip" 
						data-placement="right" 
						data-original-title="Modifica la asignatura" 
						height="16" 
						width="16">
					</a>			
				</div>
	...
	<div style="margin-bottom: 10px;">
		<?php
			$nom_prof = $assignatura->getNomProf();
			if($nom_prof != ''){
				$cognoms_prof = $assignatura->getCognomsProf();
				?>
				<em>
					<?php
						echo 'Asignatura impartida por: '.$nom_prof.' '.$cognoms_prof;
					?>
				</em>
				<br/>
				<?php
			...
			?>
				<em>
					<?php
						echo 'Curso académico: '.$curs->getNom();;
					?>
				</em>
				<br/>
			<?php
			...
			?>
				<em>
					<?php
						echo 'Moderador: '.$moderador->getNom().' '.$moderador->getCognoms();
					?>
				</em>
				<br/>
			<?php
...
?>
	...
	<?php 
		...
		echo Form::open(array(
			...
			'id' => 'form-comentar',
			'autocomplete' => 'off',
			'url' => route('assignatura.aportacio.create', array('id' => $assignatura->getID()))
		));
			
		 echo Form::textarea('aportacio', null,array('placeholder' => 'Comparte  con tus compañeros...', 
														'class' => 'form-control',
														'id' => 'boxText',														
														'required',
													   	'maxlength' => '500',
														'style' => 'width:583px; max-width:583px;'
														));?>
	</div>
	...
	<div id="aportacions">   
	   <?php
		...
		if ($aportacions->getCurrentPage() < $aportacions->getTotal()-1){
			$ruta = route('assignatura.info',
			  		array('slug' => $assignatura->getSlug()))
